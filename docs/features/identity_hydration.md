...
## Descrição
Implementação do princípio "Identity-First", onde a identidade do usuário é resolvida e carregada (hidratada) no estado do agente *antes* de qualquer processamento de intenção. Isso permite que o agente reconheça o usuário através de canais (Web, WhatsApp, etc.) caso o ID seja mapeado corretamente.
